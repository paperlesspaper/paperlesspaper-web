...
export const registerClarityIfNeeded = () => {
  ...
  if (ClarityFilterClass) return;
  const registry = (fabric as any).classRegistry;
  if (registry?.has?.("Clarity")) {
    ClarityFilterClass = registry.getClass("Clarity");
    return;
  }

  class ClarityFilter extends BaseFilter {
    static type = "Clarity";
    amount: number;
    radius: number;
    midtone: number;
  ...
  if (registry?.setClass) {
    registry.setClass(ClarityFilter, "Clarity");
  }
  ClarityFilterClass = ClarityFilter;
};
...
export default function Clarity() {
  ...
  useEffect(() => {
    ...
    const Canvas2dFB =
      ...
      (fabric as any).Canvas2DFilterBackend;
    const canvas = fabricRef?.current;
    if (Canvas2dFB && canvas) {
      canvas.filterBackend = new Canvas2dFB();
    }
