...
const lockfilePatterns = [/file:\s*\.yalc\//i, /\b\.yalc\//i, /\blink:\s*\.yalc\//i];
...
const isForbiddenSpec = (spec) => {
  ...
  return spec.includes('.yalc/') || spec.startsWith('file:') || spec.startsWith('link:');
};
...
if (violations.length > 0) {
  console.error('Detected forbidden yalc/file/link references in tracked dependency files:');
